...
namespace AvocadoShark
{
    public class Hi : EditorWindow
    {
        ...
        {
            if (SessionState.GetBool("CMP_ShowedWindow", false) == false)
            {
                if (EditorPrefs.GetBool("CMP_NeverShowAgain", false) == false)
                {
                    int startCount = EditorPrefs.GetInt("CMP_StartCount", 0);
